...
export function BlueprintJobApplicationsPage() {
    ...
    return (
        <BlueprintLayout>
            <BlueprintHeader title="Applications" subtitle={`${total} total`} actions={<div className="flex items-center gap-3"><Link to="/plugins/job-tracker"><BlueprintButton variant="ghost"><ArrowLeft className="w-4 h-4 mr-2" /> Back</BlueprintButton></Link><BlueprintButton variant="primary" onClick={() => navigate('/plugins/job-tracker/applications/new')}><Plus className="w-4 h-4 mr-2" /> Add</BlueprintButton></div>} />
            <BlueprintCard className="mb-4 !p-3">
                <div className="flex items-center justify-between">
                    ...
                    <BlueprintInput value={searchInput} onChange={setSearchInput} placeholder="Search..." icon={<Search className="w-4 h-4" />} className="w-64" />
                </div>
            </BlueprintCard>
            {loading ? <div className="flex items-center justify-center py-20"><Loader2 className="w-8 h-8 text-[var(--blueprint-cyan)] animate-spin" /></div> :
                apps.length === 0 ? <BlueprintEmpty icon={<Search className="w-8 h-8" />} text={searchInput || statusFilter !== 'all' ? 'No matches' : 'No applications'} action={<BlueprintButton variant="primary" onClick={() => navigate('/plugins/job-tracker/applications/new')}><Plus className="w-4 h-4 mr-2" /> Add</BlueprintButton>} /> :
                    <BlueprintCard className="!p-0 overflow-hidden">
                        <BlueprintTable headers={['Company', 'Position', 'Status', 'Priority', '']}>
                            {apps.map((a) => (<tr key={a.id}><td className="font-medium">{a.company}</td><td className="text-[var(--blueprint-text-dim)]">{a.jobTitle}</td><td><BlueprintBadge variant={statusColors[a.status] || 'default'}>{a.status}</BlueprintBadge></td><td><BlueprintBadge variant={a.priority === 'high' ? 'red' : a.priority === 'medium' ? 'orange' : 'default'}>{a.priority}</BlueprintBadge></td><td><Link to={`/plugins/job-tracker/applications/${a.id}`}><BlueprintButton variant="ghost">Edit</BlueprintButton></Link></td></tr>))}
                        </BlueprintTable>
                    </BlueprintCard>}

            {apps.length > 0 && (
                <div className="mt-4 pt-4 border-t border-[var(--blueprint-line-dim)]">
