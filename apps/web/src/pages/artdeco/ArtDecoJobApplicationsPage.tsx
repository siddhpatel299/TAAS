    ...
    return (
        <ArtDecoLayout>
            ...
            <DecoCard className="!p-4 mb-6 flex items-center justify-between">
                <div className="flex gap-2">{statuses.map(s => (<button key={s} onClick={() => setFilter(s)} className={cn("deco-btn !py-2", filter === s && "deco-btn-primary")}>{s}</button>))}</div>
                <div className="relative w-64"><Search className="absolute left-4 top-1/2 -translate-y-1/2 w-5 h-5 text-[var(--deco-gold-dark)]" /><input type="text" value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search..." className="deco-input !pl-12" /></div>
            </DecoCard>
            {loading ? <div className="py-20 text-center"><Loader2 className="w-10 h-10 animate-spin mx-auto text-[var(--deco-gold)]" /></div> :
                filtered.length === 0 ? <DecoEmpty text={search || filter !== 'all' ? 'No matches' : 'No applications'} /> :
                    <DecoCard><DecoTable headers={['Company', 'Position', 'Status', 'Priority', '']}>{filtered.map((a) => (<tr key={a.id}><td className="font-medium">{a.company}</td><td>{a.jobTitle}</td><td><DecoBadge color={colors[a.status] || 'gold'}>{a.status}</DecoBadge></td><td><DecoBadge color={a.priority === 'high' ? 'rose' : 'sage'}>{a.priority}</DecoBadge></td><td><Link to={`/plugins/job-tracker/applications/${a.id}`}><DecoButton>Edit</DecoButton></Link></td></tr>))}</DecoTable></DecoCard>}
        </ArtDecoLayout>
    );
}
