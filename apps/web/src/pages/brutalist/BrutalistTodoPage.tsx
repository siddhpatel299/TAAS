...
import { BrutalistCard, BrutalistButton, BrutalistEmpty, BrutalistTitle, BrutalistBadge, BrutalistCheckbox } from '@/components/brutalist/BrutalistComponents';
            ...
            <div className="grid grid-cols-4 gap-6">
                ...
                <div>
                    <div className="flex gap-2 mb-4">
                        ...
                        <BrutalistButton variant="primary" onClick={addList}><Plus className="w-5 h-5" /></BrutalistButton>
                    </div>
                    <div className="space-y-2">
                        {lists.map(list => (
                            <BrutalistCard key={list.id} color={selectedListId === list.id ? 'inverted' : 'white'} onClick={() => setSelectedListId(list.id)} className="flex items-center justify-between">
                    ...
                    {selectedList && (
                        <>
                            <div className="flex items-center justify-between mb-4">
                                ...
                                <BrutalistBadge variant="inverted">{completedCount}/{totalCount}</BrutalistBadge>
                            </div>
                            <div className="flex gap-3 mb-6">
                                ...
                                <BrutalistButton variant="primary" onClick={addTask}><Plus className="w-5 h-5" /> Add</BrutalistButton>
                            </div>
                            {selectedList.tasks.length === 0 ? <BrutalistEmpty text="No tasks yet" /> : (
                                <div className="space-y-3">
                                    {selectedList.tasks.map(task => (
                                        <BrutalistCard key={task.id} color={task.completed ? 'gray' : 'white'} className="flex items-center gap-4">
                                            <BrutalistCheckbox checked={task.completed} onChange={() => toggleTask(task.id)} />
