...
import { BrutalistCard, BrutalistButton, BrutalistTitle } from '@/components/brutalist/BrutalistComponents';
import { jobTrackerApi } from '@/lib/plugins-api';

export function BrutalistJobApplicationFormPage() {
    const { id } = useParams();
        ...
        <BrutalistLayout>
            <div className="flex items-center justify-between mb-6">
                <div className="flex items-center gap-4">
                    <Link to="/plugins/job-tracker/applications"><BrutalistButton><ArrowLeft className="w-5 h-5" /></BrutalistButton></Link>
            ...
            <BrutalistCard>
                <div className="grid grid-cols-2 gap-6">
                    <div><label className="block font-bold uppercase text-sm mb-2">Company</label><input type="text" value={form.company} onChange={(e) => setForm({ ...form, company: e.target.value })} className="brutalist-input" /></div>
                    <div><label className="block font-bold uppercase text-sm mb-2">Job Title</label><input type="text" value={form.jobTitle} onChange={(e) => setForm({ ...form, jobTitle: e.target.value })} className="brutalist-input" /></div>
                    <div><label className="block font-bold uppercase text-sm mb-2">Status</label><select value={form.status} onChange={(e) => setForm({ ...form, status: e.target.value })} className="brutalist-input"><option value="saved">Saved</option><option value="applied">Applied</option><option value="interviewing">Interviewing</option><option value="offered">Offered</option><option value="rejected">Rejected</option></select></div>
                    <div><label className="block font-bold uppercase text-sm mb-2">Priority</label><select value={form.priority} onChange={(e) => setForm({ ...form, priority: e.target.value })} className="brutalist-input"><option value="low">Low</option><option value="medium">Medium</option><option value="high">High</option></select></div>
                    <div><label className="block font-bold uppercase text-sm mb-2">Job URL</label><input type="url" value={form.jobUrl} onChange={(e) => setForm({ ...form, jobUrl: e.target.value })} className="brutalist-input" /></div>
                    <div className="col-span-2"><label className="block font-bold uppercase text-sm mb-2">Notes</label><textarea value={form.notes} onChange={(e) => setForm({ ...form, notes: e.target.value })} rows={4} className="brutalist-input" /></div>
                </div>
            </BrutalistCard>
        </BrutalistLayout>
    );
